...
const ProtectedRouteWelcome = ({ element: Component, ...rest }) => {
  const [isWelcomeVisible, setIsWelcomeVisible] = useState(null);
  ...
  useEffect(() => {
    const checkShowWelcome = async () => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:3005/api/users/protectedRouteWelcome', {
          method: 'POST',
          headers: {
...
export default ProtectedRouteWelcome;
