...
const ProtectedRouteGame = ({ element: Component, level, ...rest }) => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:3005/api/users/protectedRouteGame', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            token: token, // Send token in authorization header
          },
          body: JSON.stringify({ userId, userFullName, level }), // Send userId and userFullName in the request body
        });
  ...
  return isAuthenticated ? (
    <Component {...rest} level={level} />
  ) : (
    <Navigate to="/learningpath" replace />
  );
};
